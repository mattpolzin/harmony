...
label : Config => Git => Octokit =>
     ...
     -> Promise' ()
label @{config} labels =
  do Real _ openPr <- identifyOrCreatePR !currentBranch
...
pr {isDraft} labelSlugs =
  if all isHashPrefix labelSlugs
     then do Real actionTaken pr <- identifyOrCreatePR {isDraft} !currentBranch
...
request args {dry} = do
  ...
  if (null forcedReviewers && null teamNames)
     ...
     else do Real _ openPr <- identifyOrCreatePR !currentBranch
